...
当前本地历史重放（v9，2026-07-01）：全赢 32、半赢 5、走水 4、半输 8、全输 8，净收益 **+20.282u / 57 注，ROI 35.6%**。本轮保留 `必发指数` 作为市场热度/赔付压力上下文，不直接进入方向加权；输出统一为上盘/下盘二选一，并只用轻微/中等/强烈分级。浅盘反热门新增价值确认保护：热门有风险不自动等于下盘有价值；弱退盘、快照/成交/欧赔回上盘的浅盘下盘会回拉为轻微上盘，但盘口高点回撤且公司强确认、或成交/欧赔/市场形成强下盘簇时保留真实下盘趋势，避免误伤 1315859、1315880、1277374 这类样本。历史重放仍属于样本内验证，正式结论以后续冻结版本 walk-forward 为准。
...
| 1315854 | 捷克 vs 南非 | 无投注（旧口径） | 1-1 | na（平局） |
| 1315857 | 美国 vs 澳大利亚 | 上盘 | 2-0 | hit |
| 1315858 | 土耳其 vs 巴拉圭 | 无投注（旧口径） | 0-1 | na |
...
| 1315894 | 沙特 vs 乌拉圭 | 无投注（旧口径，模型上盘） | 1-1 | na |
| 1316314 | 乌兹别克斯坦 vs 哥伦比亚 | 上盘 | 1-3 | hit |
| 1316320 | 加纳 vs 巴拿马 | 上盘 | 1-0 | hit |

**统计**：有方向 **11** 场，**命中 8 / 未中 3**，命中率 **73%**；无投注/走水 **3**。

### 无投注场次（旧复盘口径）

| event_id | 说明 |
|----------|------|
| 1315854 | 捷克 -0.5 平南非：综合分近阈值、欧赔偏下盘，旧口径不下注以避免浅盘平局噪声。 |
| 1315858 | 土耳其 -0.5 负：模型与购买门控偏谨慎，旧口径不追浅热上盘。 |
| 1315894 | 沙特受让对乌拉圭平局：模型偏上盘但购买门控为无投注（与临场门槛类信号一致时计入 na）。 |
