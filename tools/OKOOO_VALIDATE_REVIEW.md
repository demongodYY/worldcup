...
当前本地历史重放（v7，2026-07-01）：全赢 27、半赢 5、走水 4、半输 8、全输 10，净收益 **+13.542u / 54 注，ROI 25.1%**。本轮将 `必发指数` 从直接方向加权项改为市场热度/赔付压力上下文，只供市场确认、资金/盘口弹性和热门陷阱逻辑读取；它不再单独回答“买上盘还是下盘”。历史重放仍属于样本内验证，正式结论以后续冻结版本 walk-forward 为准。
...
5. **浅盘热门陷阱**：热门方热度强、盘口只在浅盘区间、亚盘/欧赔/市场平衡没有形成确认簇时，在模型 score 层压分，典型修复样本是 1315858（土耳其 vs 巴拉圭）从追浅热上盘转为下盘。
6. **深盘强队临场修复**：深盘强队最后阶段 score 明显回升、欧赔/Kelly仍挺上盘且成交未同步反向时，在最后两条中位数验证阶段给临场点更高权重；典型修复样本是 1315871（巴西 vs 海地），避免被较早的退盘噪声拖成下盘。
7. **阈值**：`UPPER_THRESHOLD` 保持 **0.12**，避免浅盘大热（如土耳其）被抬成上盘。
8. **一球走水带**：`purchase_decision_from_signals` — 盘口为 **+1**（上盘=客场让球方）、赢盘门槛与平局风险已预警且综合分未拉开时，购买侧 **不强吃上盘**，减少净胜 1 走水类推荐。
