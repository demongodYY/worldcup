...
3. **亚盘合理性合并**：先用欧赔/必发价格估算合理盘口，再看实际盘口偏深或偏浅，最后放到 `亚盘水位` 的解释里一起阅读；澳客源下 `盘口合理性` 不再作为独立主信号输出或加权，避免和亚盘水位重复计分。
4. **亚盘水位与盘口变化一致性**：水位升降不单独判断，先扣除盘口变化带来的正常水位漂移；升盘后的正常升水、降盘后的正常降水不会被当成强背离。只有扣除盘口变化后的异常升水/降水，才会进入亚盘水位、资金/盘口弹性、历史快照和赢盘门槛风险。
5. **澳客核心信号瘦身与亚盘主导**：澳客源下，基础加权只保留资金热度、成交、亚盘水位、欧赔/Kelly、公司一致性、风险门槛、快照和资金/盘口弹性；`盘口合理性`、`盘口深度/打穿能力`、`外部赔率/实力校验`、`高低水价值` 不再作为独立主信号输出或加权，只作为内部门控/保护视角使用。最终推荐是亚盘选择，所以亚盘水位、公司一致性和资金/盘口弹性权重高于静态必发和欧赔。
...
- 澳客基础加权信号会进入 `weighted_score`：`必发指数`(0.12)、`必发成交走势`(0.09)、`亚盘水位`(0.20，解释中包含盘口合理性参考)、`欧赔/Kelly`(0.06)、`市场平衡/背离`(澳客源 0.055)、`平局风险`(澳客源 0.040)、`公司一致性`(0.10)、`赢盘门槛风险`(澳客源 0.060)、`快照趋势`(0.060)、`资金/盘口弹性`(0.080)。
- `市场平衡/背离` 是澳客主干的顺从/背离汇总：主轴是必发交易热度出现后，亚盘水位、公司一致性、成交走势和资金/盘口弹性是否承认同一方向；必发价格、欧赔/Kelly、内部盘口合理性和平局风险只做佐证或风险门控。亚盘水位、公司一致性和资金/盘口弹性是最终亚盘推荐的主判据；必发和欧赔更多用于确认胜负/热度，不能单独替代赢盘确认。
